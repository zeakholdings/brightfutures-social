...
import { e as Route } from "./router-tuXOV3p3.js";
import "lucide-react";
...
import "zod";
const formatDate = (event) => event.dateLabel || (event.startDate ? new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "long", year: "numeric" }).format(new Date(event.startDate.includes("T") ? event.startDate : `${event.startDate}T12:00:00`)) : "To be confirmed");
function EventCard({ event, compact = false }) {
  ...
  return /* @__PURE__ */ jsxs("article", { className: `group relative border-t-2 border-forest py-7 transition-transform hover:translate-x-1 ${compact ? "lg:grid lg:grid-cols-[10rem_1fr_auto] lg:gap-8" : "flex h-full flex-col"}`, children: [
    ...
    /* @__PURE__ */ jsx(Link, { to: "/events/$slug", params: { slug: event.slug }, className: "mt-5 inline-flex h-fit w-fit border-b-2 border-forest pb-1 text-sm font-bold text-forest hover:text-coral lg:mt-0", children: "View event →" })
  ] });
}
