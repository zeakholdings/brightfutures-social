...
type JsonFunctionField<Item> = `json(${Extract<LiteralFields<Item, "json">, string>}, ${string})`;
/**
...
type ReplaceChar<S extends string, From extends string, To extends string> = S extends `${infer B}${From}${infer A}` ? `${B}${To}${ReplaceChar<A, From, To>}` : S;
...
*/
type NormalizeJsonPath<S extends string> = StripTrailingUnderscore<CollapseUnderscores<ReplaceChar<ReplaceChar<ReplaceChar<S, ".", "_">, "[", "_">, "]", "_">>>;
/**
...
*/
type JsonFieldAlias<S extends string> = S extends `json(${infer Field}, ${infer Path})` ? `${Field}_${NormalizeJsonPath<Path>}_json` : never;
type QueryFunctions = {
...
*/
type FunctionFields<Schema, Item> = { [Type in keyof QueryFunctions]: TypeFunctionFields<Item, Type> }[keyof QueryFunctions] | keyof TranslateFunctionFields<RelationalFunctions<Schema, Item>, ArrayFunctions> | JsonFunctionFieldPartial<Item> | JsonFunctionField<Item>;
/**
*
*/
...
*/
type JsonFunctionFieldAliases<Item> = { [K in JsonFunctionField<Item>]: never };
/**
* Map json function fields to their original field name (the first argument)
*/
type JsonFunctionFieldOrigins<Item> = { [K in JsonFunctionField<Item>]: K extends `json(${infer F}, ${string})` ? F : never };
/**
* Map all possible function fields on an item
*/
type MappedFunctionFields<Schema, Item> = Merge<TranslateFunctionFields<RelationalFunctions<Schema, Item>, ArrayFunctions>, TranslateFunctionFields<LiteralFields<Item, "datetime">, DateTimeFunctions> & TranslateFunctionFields<LiteralFields<Item, "date">, DateFunctions> & TranslateFunctionFields<LiteralFields<Item, "time">, TimeFunctions> & TranslateFunctionFields<LiteralFields<Item, "json" | "csv">, ArrayFunctions> & JsonFunctionFieldAliases<Item>>;
/**
...
*/
type MappedFieldNames<Schema, Item> = Merge<FunctionFieldNames<RelationalFunctions<Schema, Item>, ArrayFunctions>, FunctionFieldNames<LiteralFields<Item, "datetime">, DateTimeFunctions> & FunctionFieldNames<LiteralFields<Item, "date">, DateFunctions> & FunctionFieldNames<LiteralFields<Item, "time">, TimeFunctions> & FunctionFieldNames<LiteralFields<Item, "json" | "csv">, ArrayFunctions> & JsonFunctionFieldOrigins<Item>>;
//#endregion
export { ArrayFunctions, DateFunctions, DateTimeFunctions, FunctionFields, JsonFieldAlias, JsonFunctionField, MappedFieldNames, MappedFunctionFields, PermuteFields, QueryFunctions, TimeFunctions, TypeFunctionFields };
