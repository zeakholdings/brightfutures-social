...
import { safeCmsRequest } from "./client"
...
export async function readPublishedArtWall(): Promise<ArtWallSubmission[]> {
  return safeCmsRequest("art wall", () => artWallReadClient().request(readItems("art_wall_submissions", { fields: [...fields], filter: { moderation_status: { _eq: "approved" }, removed_at: { _null: true }, published_at: { _nnull: true } }, sort: ["-featured", "-published_at"], limit: 100 })), [], (value): value is CmsArtWall[] => Array.isArray(value)).then((items) => items.map(publicWork))
}
export async function readPublishedArtWallItem(slug: string): Promise<ArtWallSubmission | null> {
  const result = await safeCmsRequest("art wall work", () => artWallReadClient().request(readItems("art_wall_submissions", { fields: [...fields], filter: { slug: { _eq: slug }, moderation_status: { _eq: "approved" }, removed_at: { _null: true } }, limit: 1 })), [], (value): value is CmsArtWall[] => Array.isArray(value))
